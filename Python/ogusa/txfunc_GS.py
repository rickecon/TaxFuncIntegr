'''
------------------------------------------------------------------------
This script is similar to txfunc.py, but estimates different tax functions.
In particular, the functions estimated here are functions of total income
and not capital and labor income separately.
...
def wsumsq(params, *args):
    ...
    txrates_est = ((coef1*(I - (I**(-coef2) + coef3)**(-1/coef2)))/I)
...
def txfunc_est(df, s, t, rate_type, output_dir, graph):
    ...
    max_init = np.minimum(txrates.max(), 0.9)
    min_init = np.maximum(txrates.min(), -0.6)
    ...
    Atil, Btil, Ctil = params_til.x
    print 'Parameter estimates: ', params_til.x
    print 'SSE: ', params_til.fun
    ...
    #     shift_y, shift, share])
    params = np.array([Atil, Btil, Ctil])
...
def get_tax_func_estimate(baseline=False, analytical_mtrs=False,
  age_specific=False, start_year=2016, reform={}, guid=''):
    ...
    if baseline:
        baseline_pckl = "TxFuncEst_GS_baseline{}.pkl".format(guid)
        pkl_path = os.path.join(TAX_ESTIMATE_PATH, baseline_pckl)
    else:
        policy_pckl = "TxFuncEst_GS_policy{}.pkl".format(guid)
